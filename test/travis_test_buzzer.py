#!/usr/bin/env python
import unittest, rostest, actionlib
import rosnode, rospy
import time
from std_msgs.msg import UInt16
from pimouse_ros.msg import MusicAction, MusicResult, MusicFeedback, MusicGoal

class BuzzerTest(unittest.TestCase):
    def setUp(self):
        self.client = actionlib.SimpleActionClient("music", MusicAction)
        self.feedback_value = []

    def test_node_exist(self):
        nodes = rosnode.get_node_names()
        self.assertIn('/buzzer',nodes, "node does not exist")

    def test_put_value(self):
        time.sleep(3)
        pub = rospy.Publisher('/buzzer', UInt16)
        for i in range(10):
            pub.publish(1234)
            time.sleep(0.1)

        with open("/dev/rtbuzzer0","r") as f:
            data = f.readline()
            self.assertEqual(data,"1234\n","value does not written to rtbuzzer0")

    def test_music(self):
        time.sleep(3)
        goal = MusicGoal()
        goal.freqs = [100, 200, 300, 0]
        goal.durations = [2,2,2,2]

        self.client.wait_for_server()
        self.client.send_goal(goal,feedback_cb=self.feedback_cb)
        self.client.wait_for_result()
        self.assertTrue(self.client.get_result(),"invalid result")

        time.sleep(3)
        self.assertEqual(goal.freqs,self.feedback_value[0:4],"invalid feedback:" 
                + ",".join([str(e) for e in self.feedback_value]))

        ###preemption###
        self.feedback_value = []
        self.client.send_goal(goal,feedback_cb=self.feedback_cb)
        self.client.wait_for_result(rospy.Duration.from_sec(0.5))

        self.assertFalse(self.client.get_result(),"stopped but return true")
        self.assertFalse(goal.freqs == self.feedback_value[0:4],"not stopped")

    def check_sequence(self):
        with open("/dev/rtbuzzer0","r") as f:
            data = f.readline()

    def feedback_cb(self,feedback):
        time.sleep(1)
        hz = feedback.remaining_steps * 100
        with open("/dev/rtbuzzer0","r") as f:
            data = f.readline()
            self.feedback_value.append(int(data.rstrip()))

if __name__ == '__main__':
    time.sleep(3)
    rospy.init_node('travis_test_buzzer')
    rostest.rosrun('pimouse_ros','travis_test_buzzer',BuzzerTest)
